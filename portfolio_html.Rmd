...
participation_counts_modules <- read.csv("query_outputs/participation_counts_modules.csv")
...
spider_attitudes <- read.csv("query_outputs/spider_attitudes.csv")
change_spider_attitudes <- read.csv("query_outputs/change_spider_attitudes.csv")
```
...
I have a passion for patterns - whether crocheting, sewing, observing spider webs, or coding. I have been uncovering patterns in complex datasets to tell stories that inform and innovate for the past six years. I am a data scientist with a Ph.D. from the University of Nebraska-Lincoln. My expertise lies in statistics, predictive modeling, pipeline production, data analytics, visualizations, wrangling, and geospatial analysis using languages/tools like R, Python, SQL, Unix, Git, high-throughput computing, and QGIS.

I excel at designing scalable workflows and statistical models that transform raw data into resolutions that drive decision-making and innovation. By applying advanced analytical techniques to biological systems, I have developed a deep understanding of complex, dynamic data - skills that translate to solving real-world challenges in industry. 

*Below are selected snippets of projects demonstrating my relevant experiences. The code behind the graphics and tables can be expanded throughout to showcase my skills in data wrangling, statistical analysis, and visualization using Python, R, or SQL languages.*
...
## Querying Sci-Comm Training Surveys [SQL, R]

**ONGOING**

<br>

* **Tech Stack**: SQL (PostgreSQL), R

* **Motivation**: Created a relational database in PostgreSQL for survey data completed by participants in science communication training workshops to assess outcomes.

* **Impact**: Wrote SQL queries to analyze outcomes of science communication training workshops led through collaboration with my principal investigator to understand the impact of such training on participants perspectives on science communication. 

-- Created a relational database of surveys completed by participants of a science communication training workshop called Nebraska Science Ambassadors:

[Bold indicates **primary key**; Italics indicates *foreign key*.]

<br>

<span style="color: #EB6864;">**module1, module2, module3, module4, pretest, postest**: surveys completed before the training (pretest) after each module in the workshop series (module*) and after the workshop series (postest)</span>

|               [**entry**, *recipient_id*, recorded_date, duration_s, progress, module, *question_id*, answer]

<br>

<span style="color: #EB6864;">**partcipants**: participant information</span>

|               [**recipient_id**, recipient_name, iteration]

<br>

<span style="color: #EB6864;">**questions**: questions used in the surveys</span>

|               [**question_id**, answer_type, definition1, definition2, definition3, definition4, definition5, definition6, definition7, definition8, question_type, question]


```{r load databases, echo = FALSE, results = 'hide'}
con <- dbConnect(SQLite(), ":memory:")
dbWriteTable(con, "module1", read.csv("SQL/NESA_Module_1.csv"))
dbWriteTable(con, "module2", read.csv("SQL/NESA_Module_2.csv"))
dbWriteTable(con, "module3", read.csv("SQL/NESA_Module_3.csv"))
dbWriteTable(con, "module4", read.csv("SQL/NESA_Module_4.csv"))
dbWriteTable(con, "pretest", read.csv("SQL/pre_test.csv"))
dbWriteTable(con, "posttest", read.csv("SQL/post_test.csv"))
dbWriteTable(con, "questions", read.csv("SQL/survey_questions.csv"))
dbWriteTable(con, "participants", read.csv("SQL/participants.csv"))
```

```{r module1, include = FALSE}
query <- dbGetQuery(con, "SELECT *
FROM module1 
LIMIT 5")
kable(query, caption = "module1") %>%
  kable_classic() %>% 
  kable_styling(full_width = FALSE, position = "left")
```

```{r posttest, include = FALSE}
query <- dbGetQuery(con, "SELECT *
FROM posttest 
LIMIT 5")
kable(query, caption = "posttest") %>%
  kable_classic() %>% 
  kable_styling(full_width = FALSE, position = "left")
```
...
FROM questions 
LIMIT 2")
kable(query, caption = "questions") %>%
  kable_classic() %>% 
  kable_styling(full_width = FALSE, position = "left")
```

```{r participants, include = FALSE}
query <- dbGetQuery(con, "SELECT recipient_id, '' AS recipient_name, iteration
FROM participants 
LIMIT 5")
kable(query, caption = "participants") %>%
  kable_classic() %>% 
  kable_styling(full_width = FALSE, position = "left")
```
...
-- Queried survey data using SQL in PostgreSQL to run analyses in R to assess participant outcomes from science communication training. 
...
*Three cohorts of participants completed the training (Spring 2022, Fall 2022, Spring 2023), each with four modules of information. In the post-training survey, participants selected which of the four modules they completed. This query grabs the responses and counts the number of modules that each participant said they attended.*
...
```{r query participation, echo = FALSE}
query <- dbGetQuery(con, "SELECT posttest.recipient_id, iteration, COUNT(answer) AS answer_count
FROM posttest
JOIN participants
ON posttest.recipient_id = participants.recipient_id
JOIN questions
...
ORDER BY posttest.recipient_id
LIMIT 5;")
kable(query, caption = "Only showing the first 5 rows.") %>%
  kable_classic() %>% 
...
*I can then graph the percentage and number of participants from each cohort that completed all (4) or some of the training modules.*

```{r participation graph, echo = TRUE, message = FALSE, warning = FALSE, fig.align="center", collapse = TRUE}
iteration_count <- participation_counts_modules %>% 
  group_by(iteration, answer_count) %>% 
  count() %>% 
  filter(iteration != "All") %>% 
...
iteration_count$position <- c(0.31, 0.81, 0.07, 0.57, 0.02, 0.09, 0.54, 0.98)
...
iteration_count %>% 
  ggplot() +
    ...
    scale_fill_manual("No. Modules Completed", 
                 values = four_colors) +
    xlab("") +
    ylab("") +
    theme_classic() +
...
**As time went on, cohorts were finishing fewer of the four modules.**

<br>

### Outcomes of Training {.tabset}
...
*I queried the responses to the surveys following module 1. I repeated this query for each of the four modules.*

```sql
SELECT iteration, module, module1.question_id, definition_1, definition_2, 
  definition_3, definition_4, definition_5, question, answer, 
  COUNT(module1.recipient_id) AS response
FROM training.module1
JOIN training.participants
ON module1.recipient_id = participants.recipient_id
JOIN training.questions
ON module1.question_id = questions.question_id
GROUP BY iteration, module, module1.question_id, definition_1, definition_2, 
  definition_3, definition_4, definition_5, question, answer
ORDER BY iteration, question_id, answer;
```

```{r efficacy, echo = FALSE}
query <- dbGetQuery(con, "SELECT iteration, module, module1.question_id, definition_1, definition_2, definition_3, definition_4, definition_5, question, answer, COUNT(module1.recipient_id) AS response
FROM module1
JOIN participants
ON module1.recipient_id = participants.recipient_id
JOIN questions
...
ORDER BY iteration, module1.question_id, answer
LIMIT 3")
kable(query, caption = "Only showing the first 3 rows.") %>%
  kable_classic() %>% 
  kable_styling(full_width = FALSE, position = "center", latex_options = "hold_position")
```
...
*I used R code to count the number of participants that selected each unique response which ranged from "Strongly disagree" (1) to "Strongly agree" (5). Then, I graphed the percentage of participants selecting each response for each question. I indicated the number of participants at the end of each bar.*

```{r efficacy graph, echo = TRUE, message = FALSE, warning = FALSE, fig.align="center", fig.width = 14, fig.height = 7, collapse = TRUE}
...
plot_grid(module1_graph, module2_graph, module3_graph, module4_graph, ncol=1, 
          align="v", rel_heights = c(1.5, 1, 1, 1.1))
```

<br>

**Generally, at least 75% of people felt that they learned information from each module. Participants appeared less confident in the takeaways from the fourth module, but there were no "Strongly disagree" to any statement in the last module.**

<br>

---

#### Impact on Sci-Comm Attitudes

<br>

*I queried the responses of each participant that completed pre- and post-training surveys.* 

```sql
select pretest.recipient_id, iteration, pretest.question_id, question, answer_type, 
  definition_1, definition_2, definition_3, definition_4, definition_5, 
  pretest.answer as pre_answer, posttest.answer as post_answer
...
```{r science communication, echo = FALSE}
query <- dbGetQuery(con, "select pretest.recipient_id, iteration, pretest.question_id, question, answer_type, definition_1, definition_2, definition_3, definition_4, definition_5, pretest.answer as pre_answer, posttest.answer as post_answer
FROM pretest
JOIN posttest
...
*I used a paired t-test to test whether responses to each shared question differed between pre- and post-training surveys. To graph the questions that showed a significant shift, I subtracted each pre-training response from the post-training response.*

Asterisks indicate significance level of the departure from 0 (no change) 'before / after' Bonferroni p-value correction (\*\*\* P < 0.001, \*\* P < 0.01, \* P < 0.05, . P < 0.1, NS P > 0.1).

The number to the right indicates the average answer value from the post-training survey (range 1-5).

<br>

```{r science communication graph, echo = TRUE, message = FALSE, warning = FALSE, fig.align="center", fig.width = 9, fig.height = 9, collapse = TRUE}
pre_post_10 <- pre_post %>% 
  filter(question_id == "prepost10")
t.test(x = pre_post_10$pre_answer, y = pre_post_10$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_10$pre_answer, y = pre_post_10$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_10_selected <- pre_post_10 %>% 
  ...
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_10$post_answer), 2), 
         stats = "*** / ***",
         question = "How good are you at science communication?") %>% 
  ggplot() +
    geom_point(aes(x = mean_change, y = question), color = "#7570b3") +
    geom_errorbar(aes(xmax = mean_change + 1.96 * se_change, xmin = mean_change - 1.96 * se_change, 
                      y = question), width = 0.25, color = "#7570b3") +
    geom_vline(xintercept = 0, linetype = "dashed", color = "#7570b3") +
    geom_text(aes(x = y2, y = question, label = stats), size = 4, color = "#7570b3") +
    geom_text(aes(x = y, y = question, label = post_mean), size = 4, color = "#7570b3") +
...
pre_post_11 <- pre_post %>% 
  filter(question_id == "prepost11")
t.test(x = pre_post_11$pre_answer, y = pre_post_11$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_11$pre_answer, y = pre_post_11$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_11_selected <- pre_post_11 %>% 
  mutate(change = post_answer - pre_answer) %>% 
  group_by(question_id, question) %>% 
  summarize(mean_change = mean(change),
            se_change = plotrix::std.error(change)) %>% 
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_11$post_answer), 2), 
         stats = "*** / ***",
...
pre_post_12 <- pre_post %>% 
  filter(question_id == "prepost12")
t.test(x = pre_post_12$pre_answer, y = pre_post_12$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_12$pre_answer, y = pre_post_12$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_12_selected <- pre_post_12 %>% 
  mutate(change = post_answer - pre_answer) %>% 
  group_by(question_id, question) %>% 
  summarize(mean_change = mean(change),
            se_change = plotrix::std.error(change)) %>% 
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_12$post_answer), 2), 
         stats = "* / NS",
...
pre_post_13 <- pre_post %>% 
  filter(question_id == "prepost13")
t.test(x = pre_post_13$pre_answer, y = pre_post_13$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_13$pre_answer, y = pre_post_13$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_13_selected <- pre_post_13 %>% 
  mutate(change = post_answer - pre_answer) %>% 
  group_by(question_id, question) %>% 
  summarize(mean_change = mean(change),
            se_change = plotrix::std.error(change)) %>% 
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_13$post_answer), 2), 
         stats = "** / *",
...
pre_post_14_1 <- pre_post %>% 
  filter(question_id == "prepost14_1")
t.test(x = pre_post_14_1$pre_answer, y = pre_post_14_1$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_14_1$pre_answer, y = pre_post_14_1$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_14_1_selected <- pre_post_14_1 %>% 
  mutate(change = post_answer - pre_answer) %>% 
  group_by(question_id, question) %>% 
  summarize(mean_change = mean(change),
            se_change = plotrix::std.error(change)) %>% 
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_14_1$post_answer), 2), 
         stats = "* / .",
         question = "I will be successful at engaging in science \ncommunication in the next year.") %>% 
  ggplot() +
    geom_point(aes(x = mean_change, y = question), color = "#7570b3") +
    geom_errorbar(aes(xmax = mean_change + 1.96 * se_change, xmin = mean_change - 1.96 * se_change, 
                      y = question), width = 0.25, color = "#7570b3") +
...
pre_post_14_2 <- pre_post %>% 
  filter(question_id == "prepost14_2")
t.test(x = pre_post_14_2$pre_answer, y = pre_post_14_2$post_answer, paired = TRUE)$p.value
p.adjust(t.test(x = pre_post_14_2$pre_answer, y = pre_post_14_2$post_answer, paired = TRUE)$p.value, 
         method = "bonferroni", n = 6)
pre_post_14_2_selected <- pre_post_14_2 %>% 
  mutate(change = post_answer - pre_answer) %>% 
  group_by(question_id, question) %>% 
  summarize(mean_change = mean(change),
            se_change = plotrix::std.error(change)) %>% 
  mutate(y = 1.2,
         y2 = 1, 
         post_mean = round(mean(pre_post_14_2$post_answer), 2), 
         stats = "** / *",
         question = "The public will be more accepting of science \nafter I engage in science communication.") %>% 
  ggplot() +
    geom_point(aes(x = mean_change, y = question), color = "#7570b3") +
    geom_errorbar(aes(xmax = mean_change + 1.96 * se_change, xmin = mean_change - 1.96 * se_change, 
                      y = question), width = 0.25, color = "#7570b3") +
...
plot_grid(pre_post_10_selected, pre_post_11_selected, pre_post_12_selected, pre_post_13_selected,
          pre_post_14_1_selected, pre_post_14_2_selected, ncol=1, align="v")
```

<br>

**Participants' reported an increase in perceived abilities in science communication.**

<br>

---

#### Impact on Attitudes for Spiders

<br>

*Lastly, I queried the post-training survey responses related to attitudes toward spiders, as a key part of this training involved engaging the public about research on spiders.*

```sql
SELECT posttest.recipient_id, posttest.question_id, question, answer_type, 
  definition_1, definition_2, definition_3, definition_4, definition_5, answer
FROM training.posttest
JOIN training.questions
ON posttest.question_id = questions.question_id
...
```{r spider attitudes, echo = FALSE}
query <- dbGetQuery(con, "SELECT posttest.recipient_id, posttest.question_id, question, answer_type, definition_1, definition_2, definition_3, definition_4, definition_5, answer
FROM posttest
JOIN questions
ON posttest.question_id = questions.question_id
WHERE posttest.question_id LIKE 'post26%'
...
*I graphed the percentage of participants responding to each level of agreement. The sample size is designated to the left of each result.*

```{r spider attitudes graph, echo = TRUE, message = FALSE, warning = FALSE, fig.align="center", fig.width = 11, fig.height = 5, collapse = TRUE}
six_colors = c("#1b9e77", "#e6ab02", "#d95f02", "#7570b3", "#e7298a", "#66a61e")

spider_attitudes <- spider_attitudes %>% 
  filter(answer != "NULL") %>% 
  mutate(answer = ifelse(answer == 1, "Strongly \ndisagree",
                  ifelse(answer == 2, "Somewhat \ndisagree",
                  ifelse(answer == 3, "Neither agree \nnor disagree", 
...
sum_spider_attitudes <- spider_attitudes %>% 
  group_by(question) %>% 
  summarize(total = sum(response)) %>% 
  mutate(y = 1.1)
...
ggplot() +
    geom_bar(aes(x = response, y = question, fill = answer), 
             position = "fill", stat = "identity", data = spider_attitudes) +
...
```{r change spider attitudes graph, echo = TRUE, message = FALSE, warning = FALSE, fig.align="center", fig.width = 11, fig.height = 4, collapse = TRUE}
change_spider_attitudes <- change_spider_attitudes %>% 
  filter(answer != "NULL") %>% 
  mutate(answer = ifelse(answer == 1, "Strongly \ndisagree",
...
sum_change_spider_attitudes <- change_spider_attitudes %>% 
  group_by(question) %>% 
  summarize(total = sum(response)) %>% 
  mutate(y = 1.1)
...
ggplot() +
    geom_bar(aes(x = response, y = factor(question), fill = answer), 
             position = "fill", stat = "identity", data = change_spider_attitudes) +
    geom_text(aes(x = y, y = factor(question), label = total), data = sum_change_spider_attitudes) +
    annotate(geom = 'segment', y = 1.46,  x = 0.30, xend = 1, yend = 1.46) +
    annotate(geom = 'text', y = 1.5,  x = 0.65, label = "70% Agree") +
    scale_y_discrete() +
    scale_x_continuous(labels = scales::percent) +
    scale_fill_manual("Level of Aggreement", 
                 values = six_colors) +
...
**Participants completed the training with a heightened curiosity about spiders, with 70% reporting that the training changed their attitude toward spiders.**
...
[**PUBLISHED IN CURRENT BIOLOGY**](https://doi.org/10.1016/j.cub.2025.02.041)
